...
export class Cart {
	...
	async renderCart() {
		...
		for (const productId in this.cart) {
			const productIdInt = parseInt(productId, 10); // Перетворення на число
		...
		cartDomString += `<div class="shopping_cart-checkout">
            <div class="shopping_cart-checkout-total" hx-swap="outerHTML">Total amount:<span hx-swap="outerHTML" id="total-amount">$${total.toFixed(2)}</span></div>
          </div>`;
        ...
        return `<div class="cart-container" data-id="${product.id}"> 
				 ...
				 <div class="cart-container-text">
		 			<div class="cart-title">${product.name}</div>
                  	...
                  	<div class="cart-btn-minus"><button data-id=${product.id} class="btn btn-sm minus">-</button></div>
					<div class="cart-btn-plus"><button data-id=${product.id} class="btn btn-sm plus">+</button></div>
                 </div>
                </div>`;
    }
    ...
    updateQuantitySpan() {
        this.quantitySpan.innerHTML = Object.values(this.cart).reduce((acc, quantity) => acc + quantity, 0);
    }
}
